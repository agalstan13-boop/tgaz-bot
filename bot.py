...
async def main():
    try:
        ...
        # ==== УСТАНОВКА ВЕБ-ХУКА С ПОВТОРНЫМИ ПОПЫТКАМИ ====
        ...
        async def webhook_handler(request):
            ...
            print(f"📩 Получен update: {data.get('update_id')}")
        ...
        # ==== ВАЖНО: ЗАПУСКАЕМ ОБРАБОТЧИК ОЧЕРЕДИ ====
        ...
        # ==== ВНУТРЕННИЙ ПИНГЕР (чтобы Render не усыпил бота) ====
        async def self_ping():
            while True:
        ...
        import traceback
        traceback.print_exc()
        # Даём время записать логи перед выходом
