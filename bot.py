...
from telegram.ext import Application, ContextTypes, CommandHandler, MessageHandler, filters, CallbackQueryHandler, JobQueue
import aiohttp
...
async def main():
    try:
        ...
        # СОЗДАЁМ БОТА (БЕЗ UPDATER - это важно!)
        application = Application.builder().token("7971701925:AAGr7HiOrCcnKeIxMjrLzisiX4LpSLMg-b4").updater(None).build()

        if application.job_queue is None:
            application.job_queue = JobQueue()
            application.job_queue.set_application(application)

        # ВСЕ ТВОИ ХЕНДЛЕРЫ - НИЧЕГО НЕ МЕНЯЙ!
        ...
        reset_game_locks()
        print("✅ Бот инициализирован, запускаем веб-сервер...")

        # ==== УСТАНОВКА ВЕБ-ХУКА ====
        webhook_url = os.environ.get('RENDER_EXTERNAL_URL', 'https://tgaz-bot.onrender.com')
        webhook_url = f"{webhook_url}/webhook"

        print(f"📡 Устанавливаем веб-хук на {webhook_url}")
        
        # Ждём 2 секунды перед первой попыткой
        await asyncio.sleep(2)

        # Пытаемся установить веб-хук несколько раз
        max_retries = 3
        for attempt in range(max_retries):
            try:
                ...
                print(f"✅ Веб-хук установлен (попытка {attempt+1})")
                break
            except Exception as e:
                print(f"⚠️ Ошибка установки веб-хука (попытка {attempt+1}): {e}")
                if attempt < max_retries - 1:
                    wait_time = (attempt + 1) * 3
                    print(f"Ждём {wait_time} сек перед следующей попыткой...")
                    await asyncio.sleep(wait_time)
                else:
                    print("❌ Не удалось установить веб-хук после всех попыток")
                    raise
        ...
        print(f"Последняя ошибка: {webhook_info.last_error_date} - {webhook_info.last_error_message}")

        if webhook_info.url != webhook_url:
            print("❌ ВЕБ-ХУК НЕ СОВПАДАЕТ! Переустанавливаю...")
            ...
            print("✅ Веб-хук переустановлен")
            
            # Проверяем ещё раз
            webhook_info = await application.bot.get_webhook_info()
            print(f"URL после переустановки: {webhook_info.url}")
        
        # Создаём HTTP сервер для приёма веб-хуков
        async def webhook_handler(request):
            print(f"🔥🔥🔥 WEBHOOK ВЫЗВАН! Метод: {request.method}")
            try:
                # Читаем тело запроса
                text = await request.text()
                print(f"📦 Сырые данные: {text[:200]}")
                
                # Парсим JSON
                data = await request.json()
                print(f"📩 Полный update: {json.dumps(data, indent=2, ensure_ascii=False)[:500]}")
                ...
                print(f"✅ Update отправлен в очередь")
        ...
        async def process_updates():
            print("🔄 Запущен обработчик очереди обновлений")
            while True:
                try:
                    # Ждём обновление из очереди
                    update = await application.update_queue.get()
                    print(f"📦 Достал update из очереди: {update.update_id}")
                ...
                try:
                    async with aiohttp.ClientSession() as session:
                        async with session.get(f'http://localhost:{port}/health') as resp:
                            if resp.status == 200:
                                print("✅ Self-ping OK (бот жив)")
                            else:
                                print(f"⚠️ Self-ping вернул статус {resp.status}")
                except Exception as e:
        ...
        print("✅ Бот работает через веб-хуки")
        print("✅ Обработчик очереди запущен")
        print("✅ Внутренний пингер активен (каждые 14 минут)")
        print("🚀 Render НЕ усыпит бота!")
